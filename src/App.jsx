...
// Data Lokasi CCTV Dishub (Ditambahkan Koordinat)
...
const UploadPage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-slate-400 bg-[#0a0a0a] rounded-xl border border-slate-800">
      <Video size={48} className="mb-4 text-blue-500" />
      <h2 className="text-xl font-bold text-white mb-2">Input Video Streaming</h2>
      <p className="mb-8 text-sm text-center max-w-md">Upload file video lokal atau masukkan URL RTSP kamera CCTV untuk mulai memantau.</p>

      <div className="flex flex-col gap-6 w-full max-w-md">
        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-700 border-dashed rounded-lg cursor-pointer bg-slate-900 hover:bg-slate-800 hover:border-blue-500 transition-all">
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            <Upload className="w-8 h-8 mb-3 text-slate-500" />
            <p className="mb-2 text-sm text-slate-400"><span className="font-semibold text-blue-500">Klik untuk upload video</span></p>
            <p className="text-xs text-slate-500">MP4, AVI, atau MKV (Max. 100MB)</p>
            {fileName && <p className="mt-2 text-sm font-bold text-emerald-500">{fileName}</p>}
          </div>
          <input type="file" className="hidden" accept="video/*" onChange={(e) => setFileName(e.target.files[0]?.name)} />
        </label>

        <div className="flex items-center gap-4">
          <hr className="flex-1 border-slate-700" />
          <span className="text-xs text-slate-500 uppercase font-bold tracking-widest">atau stream cctv</span>
          <hr className="flex-1 border-slate-700" />
        </div>
        ...
        <div className="flex flex-col gap-3">
          <input
            ...
            className="bg-slate-900 border border-slate-700 text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-4 outline-none transition-all"
            value={rtspUrl} onChange={(e) => setRtspUrl(e.target.value)}
          />
          <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-colors w-full shadow-lg">
            HUBUNGKAN KAMERA
          </button>
        </div>
      </div>
...
// FUNGSI MEMBUAT ICON CCTV KUSTOM
...
// KOMPONEN BARU: Peta Denpasar & Badung dengan OpenStreetMap Biasa & Interaktif
const RegionMinimap = ({ locations, activeCctv, onSelectCctv }) => {
  const centerPosition = [-8.6500, 115.2200]; // Pusat Peta (Denpasar)
    ...
    <div className="bg-[#111111] p-5 rounded-lg border border-slate-800 flex flex-col w-full mt-6 relative z-0">
      ...
      <div className="w-full h-80 rounded-lg overflow-hidden border border-slate-700 relative z-0">
        <MapContainer center={centerPosition} zoom={13} style={{ height: '100%', width: '100%', zIndex: 0 }} zoomControl={true}>
          {/* Menggunakan OpenStreetMap Standar yang Jelas dan Lengkap */}
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          />

          {/* Looping Lokasi CCTV untuk ditampilkan di Peta */}
          {locations.map((loc) => (
            <Marker
              key={loc.id}
              ...
              eventHandlers={{
                click: () => onSelectCctv(loc), // Ganti video saat Pin di-klik
              }}
            >
              <Popup>
...
function App() {
  ...
  const [logs, setLogs] = useState([
    { id: 1, time: '22:33:34 WITA', status: 'DANGER', loc: 'Simpang Teuku Umar', detail: 'Tabrakan antara motor dan mobil', img: 'https://via.placeholder.com/150' },
  ]);

  const handleDeleteLog = (id) => setLogs(prevLogs => prevLogs.filter(log => log.id !== id));
        ...
        <main className={`p-8 w-full h-screen overflow-y-auto relative transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-64' : 'ml-0'}`}>
          ...
          {selectedLog && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
              <div className="bg-slate-900 border border-slate-700 p-8 rounded-3xl max-w-lg w-full">
                ...
                <p className="mb-2"><strong>Lokasi:</strong> {selectedLog.loc}</p>
                <p className="mb-2"><strong>Waktu:</strong> {selectedLog.time}</p>
                <p className="mb-6"><strong>Detail:</strong> {selectedLog.detail}</p>
                <button onClick={() => setSelectedLog(null)} className="w-full bg-blue-600 hover:bg-blue-700 py-3 rounded-lg font-bold transition-colors">TUTUP</button>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-12 gap-6">
            <section className="col-span-12 xl:col-span-8 space-y-6">
              ...
              {/* VIDEO FEED DENGAN CONTROL BAR DAN PEMILIH LOKASI */}
